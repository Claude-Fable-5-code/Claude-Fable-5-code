...
""", encoding="utf-8")
    ...
    # ── keys.txt ────────────────────────────────────────────────
    (root_path / "keys.txt").write_text(f"""# 🔑 خزان المفاتيح والتوكنات السرية — {project_name}
# يُحدث فور ورود أو استخراج أي مفتاح أو توكن أو بيانات اعتماد
""", encoding="utf-8")
...
| `PROGRESS.md` | سجل التقدم الخطي والمربعات التفاعلية [x] والـ Changelog |
| `tasks.md` | المهام الحية الخطي والتفاصيل التراكمية |
| `memory.md` | الذاكرة الفنية — اكتشافات وحلول ودروس مستفادة |
| `keys.txt` | خزان المفاتيح والتوكنات السرية المحدثة |
...
""", encoding="utf-8")
    ...
    print(f"   الملفات: ai_state.json, PROGRESS.md, tasks.md, memory.md, keys.txt, ANCHORS.md, AGENTS.md")
    return True
...
def validate_root(project_name: str):
    ...
    required = ["ai_state.json", "PROGRESS.md", "tasks.md", "memory.md", "keys.txt", "ANCHORS.md", "AGENTS.md"]
