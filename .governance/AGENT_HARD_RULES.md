...
## Rule 38 — A merge commit is judged by the PR head it merged; "remaining=N" is printed by the tool every turn; CI verdict is the `--pr` block, again (R95, R96)
`state_gate verify` rejected `main` after PR #14 because `ai_state.json.git_commit` (`3e1fc30`) was neither `HEAD` (the merge) nor `HEAD~1` (main's old tip). The state file was correct; the gate was not merge‑aware. `allowed_state_commits()` now includes, for a 2‑parent HEAD, the second parent (PR head) and its first parent — a stranger SHA is still refused (self‑tests 12/13). Second half: the human asked "قولي كل مرة فاضل حاجة ولا خلاص". A promise to say so is prose; `state_gate open/close` now end with `remaining=N` — the count of `- [ ]` lines under `## Remaining` in `Root/PROGRESS.md`. **N=0 is the only "خلاص".** Third: the owner‑side agent called CI "100%" from the push run while the pull_request runs were red, and PR #14 was self‑merged 8 seconds after opening with `/rulesets = []`. The CI verdict is the full `ci_status.py --pr N` block (Rule 16 row, Round 15); the 300 s floor is enforced only when `.github/rulesets/main-protection.json` is imported server‑side (Rule 10 row, Round 15) — until then it is a wish, and the ledger says so.

*Rules 24–26 added in Round 11, 27–29 in Round 12 (R81/R83/R84), 30–34 in Round 13 (R85–R89; designs RECONSTRUCTED after a reset destroyed the first attempt), 35–37 in Round 14 (R90–R94; C0–C4 rebuilt from the C4 export after reset #2, SHAs intact), 38 in Round 15 (R95–R98; C0–C2 restored from the C2 archive after reset #5, C3 rebuilt), by the Genspark consultant.*
